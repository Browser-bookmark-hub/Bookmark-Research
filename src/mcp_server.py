...
TOOL_DESCRIPTIONS.update({
    "research_route": "Plan host-led quick, agentic or deep research. Ordinary agentic work starts with the current model's search/read/evaluate loop; deep or grouped work prefers available host collaboration. Exa Agent/Parallel Task MCPs and configured research APIs are optional support. Explicit provider is exclusive. Pass cumulative failed_routes only after confirmed failure; observe active/unknown runs. Returns the host process and tool steps; starts no work.",

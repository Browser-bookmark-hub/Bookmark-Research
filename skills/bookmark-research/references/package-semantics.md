...
首次 sync 保存包文件哈希与解析结果。后续 sync／默认查询前 refresh 检查哈希，更新变化文件和 item；坏 JSON、非法路径或循环引用会失败并回滚，缺失引用返回警告。原包临时不可用时默认查询报错，用户允许使用上次同步状态时可用 `refresh=false`；索引不保存可选择的画布历史版本。

部分导出中缺少某文件不意味着源库删除它，所以保留未出现文件并在 sync 中列出；一个已提供完整栏目文件内删除的 item 可以同步删除。返回的索引可能包含此前导入而本次未出现的栏目，应留意 `retained_missing_files`，不要宣称当前目录就是所有源数据。
...
最近一次同步传入的目录成为后续 `refresh` 的来源。合并了局部导出时，重建索引需要保留并重新导入所有必要的原始导出；仅从最近的局部目录无法恢复其他保留记录。当前没有按完整快照删除缺失栏目的模式。

JSON/.canvas 始终是事实来源，SQLite 保存派生状态；同步和查询时更新，没有后台网页监控。数据库、研究报告、临时别名与来源清单保存在同步包之外，避免包同步清理无关文件。读取任务不触发目录整理或生成流程。

...
- **Agentic search:** the host model decomposes targets, runs provider searches concurrently, reads and follows gaps. Omit `providers` to use automatic fallback for queries with no usable result. Inspect `unresolved_queries`, `routing.skipped_providers` and budget-limited `remaining_attempts`; preserve useful results from every provider. `result_state:empty` is a valid empty search, while `failed` needs its error checked. Use independent host work when delegation is available and allowed.
- **Deep research:** for sustained investigation or a report, read the [deep research workflow](references/deep-research.md), create or resume research records, then investigate through coverage and delivery. For a selected [professional service](references/research-services.md), check `research_services`, prepare the real request, start it once, observe its run ID, retrieve all report pages and import them before checking original sources. Follow returned `next_action` and `workflow`; selecting a route or preparing a payload is not a completed investigation. Missing API credentials do not disable native host research; preserve an explicitly requested service choice.
...
For deep tasks, include available host research MCP tools in routing observations; Exa `agent_run` and complete Parallel Task MCP lifecycles are recognized. Execute the selected workflow under existing authorization. On confirmed failure, record it, accumulate the failed `route_id` in `failed_routes`, reroute and continue in the same research session. Preserve explicit user provider choices. A running, interrupted/unknown, or cancelled job must not trigger a replacement; resume or observe its original ID. See [research services](references/research-services.md) for run tracking and report import. Stop switching when no eligible route or budget remains and report the evidence gaps.

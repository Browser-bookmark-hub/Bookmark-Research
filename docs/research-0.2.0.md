...
完整来源表见 [research-sources-0.2.0.json](research-sources-0.2.0.json)。其中 `R001`–`R207` 对应用户调研包的每个原始去重 URL，`S001`–`S018` 是沿研究线索补读的一手资料。来源表保存读取时间、失败状态、正文位置、哈希、相关性、采取或不采取的理由，以及原书签实例的定位 ID。
...
公开来源表把原文件夹概括为 RAG、网页提取、评测、搜索提供商、深度研究、CLI、插件宿主、浏览器能力、ACP 等主题。它保留 `section_id + item_id`，不复制私人标题、账户路径或私人笔记。**所有原始 URL 和原上下文**仍完整保存在本机 `/tmp/bookmark-research-0.2.0-review/input-inventory.json`；公开行用 `original_url_sha256` 与它一一对应。
